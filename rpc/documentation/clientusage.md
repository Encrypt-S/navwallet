...
import (
	...
	"github.com/btcsuite/navutil"
)

var certificateFile = filepath.Join(navutil.AppDataDir("navwallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", navutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/navwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/navwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by navwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.navwallet/rpc.cert"s;
    }();
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by navwallet).
...
var certPath = path.join(process.env.HOME, '.navwallet', 'rpc.cert');
...
```bash
$ protoc -I /path/to/aguycalled/navwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    cert_file_path = os.path.join(os.environ['HOME'], '.navwallet', 'rpc.cert')
