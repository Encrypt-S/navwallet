...
const (
	defaultCAFilename       = "navd.cert"
	defaultConfigFilename   = "navwallet.conf"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "navwallet.log"
...
var (
	navdDefaultCAFile  = filepath.Join(navutil.AppDataDir("navd", false), "rpc.cert")
	defaultAppDataDir  = navutil.AppDataDir("navwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of navd RPC server to connect to (default localhost:8334, testnet: localhost:18334, simnet: localhost:18556)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with navd"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	BtcdUsername     string                  `long:"navdusername" description:"Username for navd authentication"`
	BtcdPassword     string                  `long:"navdpassword" default-mask:"-" description:"Password for navd authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy RPC and navd authentication (if navdusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy RPC and navd authentication (if navdpassword is unset)"`
...
//
// The above results in navwallet functioning properly without any config
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.UseSPV {
		...
		if cfg.DisableClientTLS {
			...
			// If CAFile is unset, choose either the copy or local navd cert.
			if !cfg.CAFile.ExplicitlySet() {
				cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

				// If the CA copy does not exist, check if we're connecting to
				// a local navd and switch to its RPC cert if it exists.
				...
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						navdCertExists, err := cfgutil.FileExists(
							navdDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
						...
						if navdCertExists {
							cfg.CAFile.Value = navdDefaultCAFile
						}
					}
				}
			}
	...
	// If the navd username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for navd and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for navd and wallet.
	if cfg.BtcdUsername == "" {
