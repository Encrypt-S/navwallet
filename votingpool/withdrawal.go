...
func (s *WithdrawalStatus) Fees() navutil.Amount {
	return s.fees
}
...
func (o OutBailmentOutpoint) Amount() navutil.Amount {
	return o.amount
}
...
type withdrawalTx struct {
	...
	// struct field instead of a method so that it can be replaced in tests.
	calculateFee func() navutil.Amount
}
...
func newWithdrawalTx(setOptions func(tx *withdrawalTx)) *withdrawalTx {
	...
	tx.calculateFee = func() navutil.Amount {
		return navutil.Amount(1+tx.calculateSize()/1000) * feeIncrement
	}
	setOptions(tx)
	return tx
}
...
func (tx *withdrawalTx) inputTotal() (total navutil.Amount) {
...
func (tx *withdrawalTx) outputTotal() (total navutil.Amount) {
...
func (p *Pool) StartWithdrawal(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	...
	txStore *wtxmgr.Store, txmgrNs walletdb.ReadBucket, chainHeight int32, dustThreshold navutil.Amount) (
...
func (w *withdrawal) maybeDropRequests() {
	inputAmount := navutil.Amount(0)
	for _, input := range w.eligibleInputs {
		inputAmount += input.Amount
	}
	outputAmount := navutil.Amount(0)
...
func (wi *withdrawalInfo) match(requests []OutputRequest, startAddress WithdrawalAddress,
	lastSeriesID uint32, changeStart ChangeAddress, dustThreshold navutil.Amount) bool {
...
func getWithdrawalStatus(p *Pool, ns, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	startAddress WithdrawalAddress, lastSeriesID uint32, changeStart ChangeAddress,
	dustThreshold navutil.Amount) (*WithdrawalStatus, error) {
...
func getRedeemScript(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, addr *navutil.AddressScriptHash) ([]byte, error) {
...
func signMultiSigUTXO(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, tx *wire.MsgTx, idx int, pkScript []byte, sigs []RawSig) error {
	...
	redeemScript, err := getRedeemScript(mgr, addrmgrNs, addresses[0].(*navutil.AddressScriptHash))
	if err != nil {
