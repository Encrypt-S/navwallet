...
func upgradeToVersion5(ns walletdb.ReadWriteBucket, pubPassPhrase []byte) error {
	// First, we'll write out the new database version.
	...
	// Finally, we'll mechanically move over a set of keys which were
	// formerly under the main bucket, into the new scoped buckets. We'll
	// do so mechanically by obtaining a slice of all the keys that we need
	// to modify.
	keysToMigrate := [][]byte{
	...
	for _, bucketKey := range keysToMigrate {
		// Within this bucket key, we'll migrate over, then delete each
		// key.
		bucketToMigrate := mainBucket.NestedReadWriteBucket(bucketKey)
		err := bucketToMigrate.ForEach(func(k, v []byte) error {
			if err := bip44Bucket.Put(k, v); err != nil {
				return err
			}

			if err := mainBucket.Delete(k); err != nil {
				return err
			}
			return err
		})
		...
		if err := mainBucket.DeleteNestedBucket(bucketKey); err != nil {
			return err
		}
	}
