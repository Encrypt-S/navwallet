...
func (w *Wallet) MakeMultiSigScript(addrs []navutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*navutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*navutil.AddressScriptHash, error) {
	var p2shAddr *navutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = navutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*navutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
