...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := navutil.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))
	currentInputValues := make([]navutil.Amount, 0, len(eligible))

	return func(target navutil.Amount) (navutil.Amount, []*wire.TxIn,
		[]navutil.Amount, [][]byte, error) {
...
func (s secretSource) GetScript(addr navutil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb navutil.Amount) (tx *txauthor.AuthoredTx, err error) {
	...
	err = walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		changeSource := func() ([]byte, error) {
			...
			var changeAddr navutil.Address
			var err error
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := navutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []navutil.Amount) error {
