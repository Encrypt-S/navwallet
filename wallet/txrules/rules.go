...
const DefaultRelayFeePerKb navutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb navutil.Amount) navutil.Amount {
	...
	relayFee := navutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount navutil.Amount, scriptSize int, relayFeePerKb navutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb navutil.Amount) bool {
	...
	return IsDustAmount(navutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb navutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > navutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb navutil.Amount, txSerializeSize int) navutil.Amount {
	fee := relayFeePerKb * navutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > navutil.MaxSatoshi {
		fee = navutil.MaxSatoshi
	}
