...
// InputSourceError.
type InputSource func(target navutil.Amount) (total navutil.Amount, inputs []*wire.TxIn,
	inputValues []navutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []navutil.Amount
	TotalInput      navutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb navutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []navutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := navutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
